...
class SseniorSeniorReport extends StatefulWidget {
  static const String id = 'senior_daily_report';
  final String name;
  final DateTime startDate;
  final DateTime endDate;

  const SseniorSeniorReport({
    required this.name,
    required this.startDate,
    required this.endDate,
    super.key,
  ...
  @override
  State<SseniorSeniorReport> createState() => _SeniorDailyReportState();
}

class _SeniorDailyReportState extends State<SseniorSeniorReport>
  ...
  Future<void> _getToken() async {
    ...
    if (_token != null) {
      _fetchSeniorReportWithDate(widget.name, widget.startDate, widget.endDate);
    }
  }
  ...
  Future<void> _fetchSeniorReportWithDate(
    ...
    var url = Uri.parse(
        '${Config.apiUrl}/ssenior_seniorReport/$name?start_date=${start!.toIso8601String()}&end_date=${end!.toIso8601String()}');
    ...
    if (response.statusCode == 200) {
      ...
      print(response.body); // Handle error
    }
  }
